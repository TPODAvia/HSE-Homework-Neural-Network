...
with torch.no_grad():
    for data, target in test_loader:
        output, _ = lab.output_mod_test(model(data), target)
...
print(f"Mean_squared_error: {mean_squared_error(y_data, x_data)}")

rmsle = mean_squared_log_error(y_data, x_data, squared=False)
print(f'RMSLE: {rmsle}')

y_error = y_data - x_data
y_error_array = y_error.to_numpy()
y_error_flat = y_error_array.flatten()
...
predictions_df = pd.DataFrame({
    ...
    'Error': y_error_flat  # Assuming x_data is already  1-dimensional
})
...
predictions_df.drop(['True','Error'], axis = 1).to_csv(file_path, index=False)
